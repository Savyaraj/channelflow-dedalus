"""Linearly coupled LLE equations for a driven-dissipative photonic dimer.
...
class DedalusPy(DedalusInterface):
    ...
    def __init__(self):
        ...
        self.plot_basis = "cavity"

        self.perturb = 0
        self.read_perturb = 0
        self.mag_perturb = 1e-2
        ...
        self.write = 0
        self.write_h5 = 0
        self.write_filename = ("dedalus_data/dimer/N_" + str(self.N)
    ...
    def save_info(self, filename='./dedargs.asc'):
        with open(filename, 'a+') as f:
            f.write(f"\n\ncoupled_LLE.py\t{datetime.datetime.now()}\n")
    ...
    def problem_setup(self, mu=-1, muName='z_0'):
        ...
        self.variables = {"psi_1": self.psi_1, "psi_2": self.psi_2, "t": self.t}
        ...
        self.problem = de.IVP([self.psi_1, self.psi_2], time=self.t, namespace=self.namespace)
        self.problem.add_equation(
            "dt(psi_1) + (1+1j*(delta/2))*psi_1 - 1j*d_2*dphi(dphi(psi_1)) - 1j*J*psi_2 = "
            "-1j*ramp(z_0, z_0_end, t)*psi_1 + 1j*norm(psi_1)*psi_1 + f"
        )
        self.problem.add_equation(
            "dt(psi_2) + (k21+1j*(-delta/2))*psi_2 - 1j*d_2*dphi(dphi(psi_2)) - 1j*J*psi_1 = "
    ...
    def init_problem(self):
        ...
        if self.perturb:
            if self.read_perturb:
                ...
                self.psi_1['g'] += self.mag_perturb * ev_arr[4, 0, :] / np.linalg.norm(ev_arr[4, 0, :])
                self.psi_2['g'] += self.mag_perturb * ev_arr[4, 2, :] / np.linalg.norm(ev_arr[4, 2, :])
            else:
    ...
    def read_h5(self, filename, iter):
        ...
        with h5py.File(h5path, mode='r+') as file:
            len_read = len(file['tasks']['psi_1'][iter])
            self.psi_1.change_scales(len_read / self.N)
            self.psi_2.change_scales(len_read / self.N)
...
if __name__ == "__main__":
    ...
    if Dd.plot_basis == "cavity":
        ...
        psi_1_list = [np.copy(np.abs(Dd.psi_1['g'] - Dd.psi_2['g']))]
        psi_2_list = [np.copy(np.abs(Dd.psi_1['g'] + Dd.psi_2['g']))]
        psi_1_norm_list = [np.linalg.norm(np.abs(Dd.psi_1['g'] - Dd.psi_2['g'])) / np.sqrt(Dd.N)]
        psi_2_norm_list = [np.linalg.norm(np.abs(Dd.psi_1['g'] + Dd.psi_2['g'])) / np.sqrt(Dd.N)]

    t_list = [Dd.solver.sim_time]

    if Dd.write or Dd.write_h5:
    ...
    try:
        ...
        while Dd.solver.proceed:
            ...
            if Dd.solver.iteration % Dd.write_iter == 0:
                ...
                if Dd.plot_basis == "cavity":
                    ...
                    psi_1_list.append(np.copy(np.abs(Dd.psi_1['g'] - Dd.psi_2['g'])))
                    psi_2_list.append(np.copy(np.abs(Dd.psi_1['g'] + Dd.psi_2['g'])))
                    psi_1_norm_list.append(np.linalg.norm(np.abs(Dd.psi_1['g'] - Dd.psi_2['g'])) / np.sqrt(Dd.N))
                    psi_2_norm_list.append(np.linalg.norm(np.abs(Dd.psi_1['g'] + Dd.psi_2['g'])) / np.sqrt(Dd.N))
            ...
            if Dd.solver.iteration % (10 * Dd.write_iter) == 0:
                ...
                print(f"Completed iteration {Dd.solver.iteration}, 2-norm cavity 1 {n1}, cavity 2 {n2}")
        ...
        if Dd.ramp:
            ...
            ax[2].plot(psi_1_norm_list, z_vals, label=r'$\psi_1$')
            ax[2].plot(psi_2_norm_list, z_vals, label=r'$\psi_2$')
            ax[2].set_xlabel(r'$\|\psi\|_2$')
            ax[2].set_ylabel(r'$\zeta$')
        else:
            ax[2].plot(psi_1_norm_list, t_array, label=r'$\psi_1$')
            ax[2].plot(psi_2_norm_list, t_array, label=r'$\psi_2$')
            ax[2].set_xlabel(r'$\|\psi\|_2$')
            ax[2].set_ylabel(r'$t$')
        ax[2].yaxis.set_label_position("right")
        ax[2].yaxis.tick_right()
        ax[2].legend(loc="lower right", fontsize=7, markerscale=0.4, framealpha=0.4)
